...
def train(learning_rate, iters, batch, cls, dataset, n_bbox = 2, n_cell = 7, n_width = 448, n_height = 448, display = 20, threshold = 0.5, snapshot = None, pretrained_weights = None):
    ...
    for b in xrange(B):
        ...
        dw = tf.pow(tf.sub(tf.cast(tf.pow(tf.abs(pred_w), 0.5), tf.float32), tf.cast(tf.pow(tf.abs(gt_w), 0.5), tf.float32)), 2)
        dh = tf.pow(tf.sub(tf.cast(tf.pow(tf.abs(pred_h), 0.5), tf.float32), tf.cast(tf.pow(tf.abs(gt_h), 0.5), tf.float32)), 2)
    ...
    gt_cls = tf.reshape(tf.slice(pred, [0, 0, 5 * index], [-1,-1,-1]), [-1, S * S, n_class])

    pred_cls = tf.slice(pred, [0, 0, 5 * index], [-1,-1,-1])

    dcls = tf.pow(tf.sub(gt_cls, pred_cls), 2)

    dcls = tf.assign(dcls, tf.reshape(tf.reduce_sum(dcls, 2), [-1, S * S, 1]))

    dcls = tf.assign(dcls, tf.mul(tf.slice(center, [0, 0, 0], [-1,-1, 1]), dcls))

    loss = tf.assign(loss, tf.concat(2, [loss, dcls]))
    ...
    optimizer = tf.train.AdamOptimizer(learning_rate).minimize(loss, aggregation_method = tf.AggregationMethod.EXPERIMENTAL_TREE)
